...
func main() {
	...
	boat := Product{"Boat", blue, medium}
	// store products in []Product
	products := []Product{apple, tree, car, boat}
	fmt.Println(products)
	fmt.Println("Green products (old):")
	// create a filter object variable
	f := Filter{}
	for _, v := range f.FilterByClolor(products, green) {
		fmt.Printf(" - %s green\n", v.name)
	}
	// search and pintout all green products using Specification pattern
	...
	rSpec := ColorSpec{red}
	rlSpec := CombineSpec{
		first:  rSpec, // define online 131 or command // greenSpec variable
		second: lSpec,
	}
	// Print filter results from Filter method
	fmt.Println("Combine Large & Red products:")
	for _, v := range bf.Filter(products, rlSpec) {
		fmt.Printf(" - %s: red, large\n", v.name)
	}
}
